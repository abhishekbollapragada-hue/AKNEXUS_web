...
const Attendance = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.error("Admin attendance load failed:", e);
        if (!mounted) return;
        setEmployees([]);
        setSelectedId(null);
      } finally {
  ...
  // ✅ EXPORT CSV
  const exportCSV = () => {
    const rows = filtered.length ? filtered : employees;
    ...
    a.href = url;
    a.download = `admin-attendance-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            View attendance trends and exceptions across all employees.
          </p>
        </div>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                console.error("Admin attendance refresh failed:", e);
                setEmployees([]);
                setSelectedId(null);
              } finally {
                setLoading(false);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 rounded-2xl border bg-white overflow-hidden">
          ...
          {loading && (
            <div className="px-4 py-6 text-sm text-muted-foreground">Loading attendance...</div>
          )}
                            ...
                            <div className="w-28 h-2 rounded-full bg-slate-100 overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(
                                    100,
                                    Math.max(0, Number(e.monthPercent) || 0)
                                  )}%`,
                                }}
                              />
                            </div>
              ...
              <div className="mt-4 rounded-2xl border bg-slate-50 p-3">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <li className="flex items-center justify-between">
                    <span>Checkout</span>
                    <span>
                      {selected.checkOut !== "—" ? selected.checkOut : "Not checked out"}
                    </span>
                  </li>
                </ul>
              </div>
      ...
      {/* ✅ Needs attention (Mentor-style cards + Review button) */}
      <div className="mt-6 rounded-2xl border bg-white p-4">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          {employees
            .filter((e) => e.statusToday === "Late" || e.statusToday === "Absent")
            .slice(0, 3)
            .map((e) => (
              <div key={e.id} className="rounded-2xl border p-3 hover:bg-slate-50">
                <div className="flex items-center justify-between">
                  <div className="font-medium">{e.name}</div>
                  <span
                    className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${getStatusStyles(
                      e.statusToday
                    )}`}
                  >
                    {e.statusToday}
          ...
          {/* If no exceptions */}
          {employees.filter((e) => e.statusToday === "Late" || e.statusToday === "Absent").length ===
            0 && (
            <div className="text-sm text-muted-foreground">
              No exceptions today 🎉 Everyone is on track.
            </div>
          )}
        </div>
      </div>
